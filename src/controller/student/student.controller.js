...
class StudentController {
    ...
    static async addStudent(req, res) {
        try {
            ...
            if (!newParent && !newStudent) {
                return respond(res, 409, 'Student and parent not created')
            }
            return respond(res, 201, 'Student added successfully', { student: newStudent, parent: newParent })
        } catch (err) {
    ...
    static async listStudent(req, res) {
        try {
            ...
            if (!student) {
                return respond(res, 409, 'Student not retrieved')
            }
            return respond(res, 400, 'Student retrieved successfully', { student })
        } catch (err) {
    ...
    static async searchFee(req, res) {
        try {
            ...
            return respond(res, 200, 'Student Fee successfully retrieved', { student_fee: studentFee })
        } catch (err) {
            logger.error(`Failed to retrieve student fees ${err}`)
            return respond(res, 500, 'Internal server error')
        }
    ...
    static async listFeeGroup(req, res) {
        try {
            ...
            if (!feeGroup) {
                return respond(res, 409, 'FeesGroup not retrieved')
            }
